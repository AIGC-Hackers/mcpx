...
describe('registry config', () => {
	it('derives command names for cached tool schemas', () => {
		...
		expect(
			normalizeRegistryConfig(config).servers.posthog?.tools?.[0]?.commandName,
		).toBe('alert-create')
	})
	...
	it('preserves cached MCP tool metadata', () => {
		...
		expect(
			normalizeRegistryConfig(config).servers.browser?.tools?.[0],
		).toMatchObject({
	...
	it('removes a server from registry config', () => {
		...
		expect(
			removed && removed.transport !== 'stdio' ? removed.url : undefined,
		).toBe('https://mcp.posthog.com/mcp')
