...
describe('mcpxd daemon client', () => {
	...
	it('starts mcpxd on demand for stdio calls', async () => {
		...
		expect(
			(await listMcpTools(server, 'fixture')).map((tool) => tool.name),
		).toContain('echo')
	...
	it('reuses a stdio server process across separate calls', async () => {
		...
		expect(text(await callMcpTool(server, 'increment', {}, 'fixture'))).toBe(
			'1',
		)
		expect(text(await callMcpTool(server, 'increment', {}, 'fixture'))).toBe(
			'2',
	...
	it('passes cwd to managed stdio processes', async () => {
		...
		expect(
			await fs.realpath(text(await callMcpTool(server, 'cwd', {}, 'fixture'))),
		).toBe(await fs.realpath(cwd))
	})
	...
	it('routes HTTP servers through mcpxd and preserves session ids', async () => {
		...
		expect(
			(await listMcpTools(server, 'http-fixture')).map((tool) => tool.name),
		).toEqual(['echo'])
		expect(text(await callMcpTool(server, 'echo', {}, 'http-fixture'))).toBe(
			'http-ok',
		)

		expect(
			fixture.sessions.slice(1).every((session) => session === 'session-1'),
		).toBe(true)
	...
	it('retries HTTP session creation after an initial connection failure', async () => {
		...
		expect(
			(await listMcpTools(server, 'http-flaky')).map((tool) => tool.name),
		).toEqual(['echo'])
	...
	it('drops retained HTTP session ids for auth-refreshed eviction', async () => {
		...
		expect(fixture.sessions.slice(sessionsBeforeEvict)).not.toContain(
			'session-1',
		)
	...
	it('stops an incompatible daemon before starting a compatible one', async () => {
		...
		expect(
			text(await callMcpTool(fixtureServer(), 'echo', {}, 'fixture')),
		).toBe('ok')
	...
	it('reports and stops the daemon', async () => {
		...
		expect((await daemonStatus(mainPath)).servers[0]?.labels).toEqual([
			'fixture',
		])
...
function fixtureServer(
	overrides: Partial<StdioServerConfig> = {},
): StdioServerConfig {
	return {
		transport: 'stdio',
...
async function startHttpFixture(
	...
	httpFixture = http.createServer(async (request, response) => {
		...
		if (message.method === 'notifications/initialized') {
			response
				.writeHead(202, { 'mcp-session-id': sessionId ?? 'session-1' })
				.end()
			return
		}
...
async function freePort(): Promise<number> {
	...
	await new Promise<void>((resolve) => server.close(() => resolve()))
	if (!address || typeof address === 'string')
		throw new Error('Free port lookup failed.')
	return address.port
}
