...
import { normalizeAuthScheme, resolveHeaders, resolveProbeHeaders } from "../src/headers";
...
describe("headers", () => {
  ...
  it("parses literal and referenced bearer credentials", () => {
    expect(authFromBearerValues(["env:FIRST_TOKEN", "${SECOND_TOKEN}", "sk_literal"])).toEqual({
      kind: "bearer",
      credentials: [
  ...
  it("fails fast when a bearer env reference is missing", () => {
    ...
    try {
      expect(() => resolveProbeHeaders(server)).toThrow(/MISSING_MCPX_TEST_TOKEN/);
    } finally {
  ...
  it("round-robins bearer credentials across CLI invocations", async () => {
    ...
    try {
      const server = bearerServer(["env:FIRST_MCPX_TEST_TOKEN", "env:SECOND_MCPX_TEST_TOKEN"]);
