...
describe('oauth', () => {
	it('uses protected-resource scopes that are supported by the authorization server', () => {
		expect(
			chooseOAuthScope(
				['openid', 'profile', 'alert:read'],
				['openid', 'alert:read'],
			),
		).toBe('openid alert:read')
	})
	...
	it('discovers authorization server metadata for path-based issuers', async () => {
		...
		try {
			const metadata = await fetchAuthorizationServerMetadata(
				'https://supabase.example/auth/v1',
			)
	...
	it('falls back to issuer-relative metadata used by some hosted OAuth providers', async () => {
		...
		try {
			...
			expect(metadata.tokenEndpoint).toBe(
				'https://supabase.example/auth/v1/oauth/token',
			)
		} finally {
			globalThis.fetch = originalFetch
		}
