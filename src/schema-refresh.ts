...
export function isSchemaRefreshStale(
	server: ServerConfig,
	now: Date = new Date(),
): boolean {
...
export function hasStaleSchemas(
	config: RegistryConfig,
	now: Date = new Date(),
): boolean {
	return Object.values(config.servers).some((server) =>
		isSchemaRefreshStale(server, now),
	)
}
...
async function refreshOneServer(
	...
	const authResult = await ensureAuthReady(
		name,
		server,
		toolsBefore,
	...
	const schemaChanged =
		JSON.stringify(readyServer.tools ?? []) !==
		JSON.stringify(refreshed.tools ?? [])
	const status =
		authResult.authStatus === 'none'
			? 'schema-refreshed'
	...
	if (server.auth.kind === 'oauth') {
		if (!interactiveAuth) {
			const result: ServerRefreshResult = {
				...
				toolsBefore,
				message:
					'OAuth authentication is required. Run mcpx @refresh in an interactive shell.',
			}
...
async function reauthenticateOneServer(
	...
	try {
		...
		const result: ServerRefreshResult = {
			server: name,
			status: isReauthRequiredMessage(message)
				? 'reauth-required'
				: 'unreachable',
			toolsBefore,
			message,
...
function isSameRefreshTarget(left: ServerConfig, right: ServerConfig): boolean {
	...
	return (
		left.url === right.url &&
		JSON.stringify(left.headers ?? null) ===
			JSON.stringify(right.headers ?? null) &&
...
function refreshStatusFromResult(
...
function tokenChanged(
	left: OAuthToken | undefined,
	right: OAuthToken | undefined,
): boolean {
