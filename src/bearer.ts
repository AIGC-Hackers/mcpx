...
export function authFromBearerValues(
  ...
  return { kind: "bearer", credentials, strategy: "round-robin", confidence: "configured" };
}

export function bearerAuthRef(auth: Extract<AuthDiscovery, { kind: "bearer" }>): string[] {
...
export function describeBearerAuth(auth: Extract<AuthDiscovery, { kind: "bearer" }>): string {
...
export function resolveBearerHeaderForProbe(
  ...
  if (!credential) throw new Error("Bearer auth requires at least one credential.");
...
function parseEnvReference(value: string): string | undefined {
  if (value.startsWith("env:")) {
    ...
    if (!isEnvName(name)) throw new Error(`Invalid bearer env reference: ${value}`);
    return name;
  }
...
async function readCursorState(): Promise<BearerCursorState> {
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") return { version: 1, cursors: {} };
    throw error;
  }
}
...
function cursorKey(serverUrl: string, auth: Extract<AuthDiscovery, { kind: "bearer" }>): string {
