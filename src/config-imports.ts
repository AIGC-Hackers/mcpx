...
export function pathsForImport(kind: ImportKind, rootDir: string): string[] {
  switch (kind) {
    ...
    case "codex":
      return [
        path.resolve(rootDir, ".codex", "mcp.toml"),
        path.join(os.homedir(), ".codex", "mcp.toml"),
      ];
    case "windsurf":
...
export async function readExternalEntries(filePath: string): Promise<Map<string, RawEntry> | null> {
  ...
  if (filePath.endsWith(".toml")) {
    ...
    return extractFromCodexConfig(parsed as Record<string, unknown>);
  }
...
function convertExternalEntry(value: Record<string, unknown>): RawEntry | null {
  ...
  const url = asString(
    value.baseUrl ?? value.base_url ?? value.url ?? value.serverUrl ?? value.server_url,
  );
  if (url) {
    result.baseUrl = url;
  }
...
async function fileExists(filePath: string): Promise<boolean> {
  try {
    ...
    // Expected: file may not exist; return false for validation
    return false;
  }
}
