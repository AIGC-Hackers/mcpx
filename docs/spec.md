---
summary: 'Plan for the mcpx package replacing the Sweetistics pnpm MCP helpers.'
---
...
## Deliverables
- `packages/mcpx` (standalone npm package) exporting:
...
- Reuse `@modelcontextprotocol/sdk` transports; invoke stdio servers directly (e.g., call `bunx` with env overrides) without an extra wrapper script.
...
- Optional OAuth token cache directory handling (defaulting to `~/.mcpx/<server>` when none is provided).
...
- Embed schemas (via `listTools`) directly in the generated source so repeat executions avoid additional metadata calls.
...
- Render tool metadata as TypeScript Spec format: type declarations with literal `name` field and `args` object. Include JSDoc comments for descriptions. All parameters (required and optional) are always shown inline.
- Ensure output parity with existing helper.
- `call` has to parse both `server.tool` tokens and HTTP selectors like `https://host/path.tool(args)`; in the HTTP case we need to peel off the `.tool` suffix, infer/auto-register the ad-hoc server (respecting `--allow-http`), and hydrate arguments from the function-call expression in parentheses.
